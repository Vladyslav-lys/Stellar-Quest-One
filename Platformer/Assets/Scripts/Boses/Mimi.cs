...
public class Mimi : MonoBehaviour, ITakeDamage, IPlayerRespawnListener
{
   ...
   private void AttackMelle()
   {
    ...
    IEnumerator Fire()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            var projectile_1 = (Projectile)Instantiate (Projectile, new Vector3(transform.position.x, transform.position.y - 3.5f, 0), transform.rotation);
            var projectile_2 = (Projectile)Instantiate (Projectile, new Vector3(transform.position.x, transform.position.y + 2.7f, 0), transform.rotation);
            var projectile_3 = (Projectile)Instantiate (Projectile, new Vector3(transform.position.x, transform.position.y - 6.7f, 0), transform.rotation);
            ...
            projectile_2.Initialize (gameObject, _direction, _controller._velocity);
            projectile_3.Initialize (gameObject, _direction, _controller._velocity);
    ...
    {
        _speed = 0;
        SmallMimi.transform.position = transform.position;
        yield return new WaitForSeconds(2);
        SmallMimi.SetActive(true);
